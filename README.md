...
- Выбор **контекста** перед запуском на странице MR или задачи: новый чат или продолжение одной из прежних сессий агента
  по этому объекту (видны тип, агент, накопленные токены). В списках бейдж «контекст #N» отмечает, что такая сессия есть.
...
6. **Продолжение сессии.** Session id агента сохраняется. «Проверить изменения» продолжает сессию ревью автоматически;
   на страницах MR и задачи перед запуском можно выбрать контекст: «Новый чат» или «Продолжить сессию #N» (тот же агент,
   тот же каталог, `--resume`). «Вопрос агенту» тоже вызывает `--resume` в том же каталоге.

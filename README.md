...
1. **Merge requests** — синхронизируйте список; у каждого MR одно главное действие по его состоянию:
   «Запустить ревью» → «Открыть замечания» → после новых коммитов «Проверить изменения». Остальное в меню ⋯.
2. **Задачи** — «Решить задачу» отдаёт задачу агенту целиком в отдельном workspace; «Исследовать» даёт план
   без изменений кода, после чего появляется «Реализовать этот план».
3. **Сессия** — каждая операция агента открывается на своей странице: состояние, результат, workspace с diff,
   продолжение диалога с агентом, технический лог.
...
| **Исследовать** | Read-only анализ: шаги, файлы и что в них меняется, риски, открытые вопросы, оценка размера. Поле «Указания» позволяет уточнить, как решать. После исследования главное действие — **Реализовать этот план**: план передаётся агенту автоматически. |

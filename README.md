...
| **Исправить выбранные** (свои MR) | Чекбоксы у открытых замечаний AI-ревью и у нерешённых обсуждений. Агент в workspace ветки MR перепроверяет каждое выбранное замечание, исправляет подтверждённые, учитывает обсуждения, прогоняет тесты, делает self-review; итог по каждому замечанию записывается на него. Дальше Commit / Push и «Проверить изменения». | Не коммитит, не пушит, в GitLab не пишет. |
...
| **Проверить замечание** (меню ⋯ у открытого замечания) | Отдельный мини-запуск по одному замечанию: агент перепроверяет его по коду, не считая верным априори, и выносит решение подтверждено / ложное срабатывание / неактуально / недостаточно данных с доказательством. Ложное и неактуальное закрывают замечание. | Read-only, остальной MR не трогается. |
| **Разобрать ошибки CI** (экран CI, карточка на MR) | Read-only: агент получает упавшие jobs head pipeline и хвосты их логов, сопоставляет с diff MR и кодом и объясняет каждое падение: тип (код MR, тест, флак, инфраструктура), причина, исправление, можно ли исправить в MR. | Ничего не меняет. |
| **Исправить CI** (только свои MR) | Агент в workspace ветки MR исправляет причины падения pipeline, прогоняет относящиеся тесты, делает self-review; что исправить в MR нельзя — перечисляет с причиной. Разбор, если был, передаётся ему. | Не коммитит и не пушит сам. |
| **Проверить на стенде** | Агент в workspace ветки MR через skill доступа к стенду проекта (`STAND_SKILL`, ищется по слову «стенд») заливает изменённые файлы MR на стенд, прогоняет там тесты по затронутому коду, пишет скрипт-эмуляцию функциональности MR с моками внешних систем, запускает его и отчитывается: что залито, тесты, вывод скрипта, проблемы. Скрипт остаётся в workspace. | Не меняет код MR, не трогает git и БД на стенде, не пишет в GitLab. |
| **Вопрос агенту** | Продолжение той же сессии агента по результату: уточнить, оспорить, попросить расписать фикс. | Read-only. |
| **Просмотрено** | Пометка для длинного списка: MR затемняется и уходит в конец. Снимается сама, когда синхронизация приносит новые коммиты, комментарии или изменения в обсуждениях; вручную — «Снять отметку». | В GitLab ничего не меняется. |
...
| **Исследовать** (режим «Баг») | Для задач-багов: ожидалось / фактически, воспроизведение, путь выполнения, root cause с доказательствами, предлагаемое исправление; далее «Исправить». Режим выбирается заранее по меткам и названию, переключается в форме. |
...
| **Довести до MR** | Чек-лист на странице реализации (реализация, тесты, self-review агента, чистый diff, push) и форма MR с описанием из отчёта агента; «Создать MR» активна после Push. |
...
- **Свои skills без правок проекта**: на странице проверки окружения у каждого действия можно выбрать другой skill
  проекта или включить «свои инструкции» и написать их прямо в dashboard; они подставляются в промпт вместо skill,
  хранятся в `data/` и применяются без перезапуска.
- **Продолжить в терминале**: любая завершённая сессия агента открывается в окне терминала на этой же машине
  (`claude --resume <id>` в каталоге сессии, для Codex `codex resume`). Там действуют ваши обычные права агента, а не
  ограничения dashboard. Рядом «Открыть терминал в каталоге сессии» и «Скопировать команду» для работы с другой машины.
  Эмулятор определяется автоматически, задаётся `TERMINAL_CMD`.
- Выбор **контекста** перед запуском на странице MR или задачи: новый чат или продолжение одной из прежних сессий агента
  ...
  Рядом с выбором агента — **расход подписки агента**: чипы «5 ч», «неделя», недельный лимит выбранной модели
  (Claude), «доп. кредиты»; для Codex — его 5-часовое и недельное окна. Данные берутся с usage-эндпоинтов самих
  агентов по их локальным учётным данным (`~/.claude/.credentials.json`, `~/.codex/auth.json`), ничего не
  сохраняется и не обновляется dashboard'ом; период опроса `USAGE_POLL_MIN` (0 = выключить).
  Рядом с выбором агента — выбор **модели**: «авто» (модель самого агента по его настройкам) или одна из
  `CLAUDE_MODELS` / `CODEX_MODELS` (например, Fable и Opus для Claude; последние модели Codex). Выбор запоминается
  в браузере отдельно для каждого агента, передаётся как `--model` / `-m`, показывается в шапке сессии и
  сохраняется при «Повторить» и продолжении сессии.
...
- У каждого запуска: полный лог (команда, промпт, stdout/stderr агента), израсходованные токены с разбивкой
  ...
  Доллары не показываются: при работе по подписке важны лимиты в токенах. Для сессий Claude Code показан
  **процент заполнения контекста** (живой во время запуска и итоговый; он же в выборе сессии для продолжения).
...
3. **Промпт dashboard** добавляет только то, что нужно интерфейсу: ссылку и head SHA, режим, требование
   read-only (или правила работы в worktree) и JSON-схему результата, плюс предзагруженный контекст MR: список
   изменённых файлов, diff (до лимита) и нерешённые обсуждения, чтобы агент не тратил вызовы на их получение.
   Правила проекта не дублируются. Ревью выполняется в read-only worktree на точном head SHA MR.
...
| `REVIEW_WORKTREE` | `1` | Ревью, проверка замечания и разбор CI идут в read-only worktree на head SHA MR; `0` = из корня проекта |
| `REVIEW_WORKTREE_TTL_MIN` | `60` | Через сколько минут без сессий review-worktree удаляется (проект большой, место на диске). Удаляется и сразу, когда MR влит, закрыт, скрыт или удалён; следующее ревью создаёт заново, вопрос агенту восстанавливает. `0` = только вручную на странице Workspaces |
| `PREFETCH_MAX_DIFF_CHARS` | `60000` | До какого размера diff MR вставляется в промпт ревью целиком (иначе только список файлов) |
| `STAND_SKILL` | auto | Skill проекта с доступом к стенду для «Проверить на стенде»; пусто = первый skill, в имени или описании которого есть «стенд»/«stand» |
| `TERMINAL_CMD` | auto | Как открыть окно терминала для «Продолжить в терминале»: программа и аргументы с `{dir}` и `{cmd}`. Пусто = найти gnome-terminal, konsole, xfce4-terminal, kitty, alacritty, wezterm или xterm |
...
| «Продолжить в терминале» неактивно | Doctor → строка `Terminal`: нет эмулятора (задайте `TERMINAL_CMD`) или сервер запущен без графической сессии. Запасной путь — «Скопировать команду для терминала». |

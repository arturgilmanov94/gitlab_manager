...
Тот же вид запуска проверяет ветку решения задачи (с карточки задачи или страницы решения): вход — задача вместо MR, worktree
на ветке реализации, список файлов — `git diff --name-status origin/<base>` плюс untracked из `git status --short` (в workspace
могут быть незакоммиченные правки); дополнительный шаг — вердикт в `summary`, работает ли решение. Найденные ошибки — в
`problems[]`, не в правках кода.
